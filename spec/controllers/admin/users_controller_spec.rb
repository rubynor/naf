...
describe Admin::UsersController do
  ...
  context "super admin" do
    before :each do
      ...
      @signed_in_user = Fabricate(:user, :super_admin => true)
    ...
    it "renders all users for a region" do
      ...
      User.should_receive(:where).with(:_id => @signed_in_user.id).and_return(@signed_in_user)
      User.should_receive(:where).with(:region_id => @region1.id.to_s).once.and_return(@users)
